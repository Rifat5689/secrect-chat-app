  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("message:send", async (data, callback) => {
      try {
        ...
        if (!receiverId || (!hasText && !hasFile)) {
          return callback?.({ success: false, error: "Receiver ID and message content are required." });
        }
    ...
    socket.on("message:react", async (data, callback) => {
      try {
        ...
        if (!isParticipant) {
          return callback?.({ success: false, error: "You cannot react to this message." });
        }
        ...
        // Notify self
        socket.emit("message:reaction", responsePayload);

        // Notify target user
        const targetId = message.sender.toString() === userId ? message.receiver.toString() : message.sender.toString();
        const targetSocketId = onlineUsers.get(targetId);
        if (targetSocketId) {
          io.to(targetSocketId).emit("message:reaction", responsePayload);
        }
